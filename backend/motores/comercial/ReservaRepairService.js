 ...
 *
 * Fase 2 / Implementação 03.7:
 *   escritores de reservado_fiscal → reservasPublico
 *   (reservarQuantidade / liberarQuantidadeReservada).
 * Tracking permanece em pedido_estoque_reservas. Somente fiscal (comportamento
 * encontrado). Não altera saldo físico. Não abre transação própria.
 *
...
function normalizarOptions(options = {}) {
  return {
    ...
    empresaId: options.empresaId != null ? options.empresaId : options.empresa_id,
    exigirEmpresa: options.exigirEmpresa === true,
    motivoCompat: options.motivoCompat || null,
    usuarioId: options.usuarioId != null ? options.usuarioId : options.usuario_id,
    req: options.req || null,
    ctx: options.ctx || null,
    validarEmpresa: options.validarEmpresa
  };
}
...
function montarOptsPortaReservaRepair(fonte = {}) {
  const empresaId = resolverEmpresaId(fonte)
    ...
    ?? resolverEmpresaId(fonte.req)
    ?? resolverEmpresaIdDaRequisicao(fonte.req)
    ?? resolverEmpresaIdDaRequisicao(fonte.contexto)
    ?? resolverEmpresaIdDaRequisicao(fonte.ctx);

  const base = {
    db: getDb(fonte.db),
    usuarioId: fonte.usuarioId != null
      ? fonte.usuarioId
      : (fonte.usuario_id != null ? fonte.usuario_id : null),
...
function fontePortaRepair(ctx = {}, opts = {}) {
  return {
    ...
    ...ctx,
    db: opts.db,
    contexto: opts.contexto || ctx,
    req: opts.req || ctx.req,
    usuarioId: opts.usuarioId != null
      ? opts.usuarioId
      : (ctx.usuario_id != null || ctx.usuarioId != null
        ? (ctx.usuario_id ?? ctx.usuarioId)
        : null),
    empresaId: opts.empresaId != null
      ? opts.empresaId
      : (ctx.empresaId != null || ctx.empresa_id != null
        ? (ctx.empresaId ?? ctx.empresa_id)
        : undefined)
  };
}

/**
 * Aplica o sinal real da operação via porta pública.
 * Somente reservado_fiscal — o Repair não escreve reservado_nao_fiscal.
 */
async function aplicarDeltaReservadoFiscal(produtoId, quantidade, sentido, fonte) {
  const optsPorta = montarOptsPortaReservaRepair(fonte);
  const fn = sentido === 'liberar'
    ? reservasPublico.liberarQuantidadeReservada
    : reservasPublico.reservarQuantidade;
  await fn(produtoId, TipoSaldo.FISCAL, quantidade, optsPorta);
  return optsPorta;
}

function anexarContextoEmpresa(detalhes, optsPorta) {
  if (!optsPorta || detalhes == null) return detalhes;
  return {
    ...detalhes,
    empresa_id: optsPorta.empresaId != null ? optsPorta.empresaId : null,
    legado: optsPorta.legado === true,
    motivo_compat: optsPorta.legado === true ? (optsPorta.motivoCompat || null) : null
  };
}
...
async function handlerLiberarReserva(ctx = {}, opts = {}) {
  ...
  // 4–5) Liberar reserva (↓ reservado_fiscal via porta) e cancelar tracking
  ...
  return resultado({
    ...
    risco,
    dryRun: false,
    detalhes: anexarContextoEmpresa({
      pedido_id: pedidoIdEfetivo,
      produto_id: produtoId,
      reserva_id: reservaId,
...
async function handlerRemoverReserva(ctx = {}, opts = {}) {
  ...
  // 4–5) Cancelar reserva órfã (↓ reservado_fiscal via porta) e tracking
  const optsPorta = await aplicarDeltaReservadoFiscal(
    produtoId,
    qtd,
    'liberar',
...
async function handlerAjustarReserva(ctx = {}, opts = {}) {
  ...
  let qtdDelta;
  if (diferenca > 0) {
    // Aumento: precisa de disponível suficiente → reservarQuantidade
    const disponivel = round3(saldoFiscal - reservadoAntes);
    ...
    // Redução: reservado deve cobrir a diferença absoluta → liberarQuantidadeReservada
