...
function responderErroVinculo(res, err) {
  const code = err && err.code ? err.code : 'ERRO_VINCULO';
  ...
  const payload = {
    error: err && err.message ? err.message : 'Erro ao processar vínculo empresarial.',
    code
  };
  if (err && err.usuario_id != null) payload.usuario_id = err.usuario_id;
  if (err && err.empresa_id != null) payload.empresa_id = err.empresa_id;
  if (status >= 500) console.error('Erro em vínculo usuário-empresa:', err);
  return res.status(status).json(payload);
}

router.get('/usuarios/:id/empresas', exigirAdmin, exigirContexto, async (req, res) => {
  try {
    const lista = await UsuarioEmpresaService.listarVinculosDoUsuario(req.params.id, { db });
    return res.json(lista);
  } catch (err) {
    return responderErroVinculo(res, err);
  }
});

router.post('/usuarios/:id/empresas', exigirAdmin, exigirContexto, async (req, res) => {
  try {
    const empresaId = exigirEmpresaAlvoDoContexto(req.empresaId, req.body || {});
    const vinculo = await UsuarioEmpresaService.vincularUsuarioEmpresa(
      req.params.id,
      { empresaId },
      { db }
    );
...
router.patch('/usuarios/:id/empresas/:empresaId', exigirAdmin, exigirContexto, async (req, res) => {
  try {
    const empresaId = exigirEmpresaAlvoDoContexto(req.empresaId, req.params.empresaId);
    ...
    const vinculo = await UsuarioEmpresaService.inativarVinculo(
      req.params.id,
      { empresaId },
      { db }
    );
...
router.delete('/usuarios/:id/empresas/:empresaId', exigirAdmin, exigirContexto, async (req, res) => {
  try {
    const empresaId = exigirEmpresaAlvoDoContexto(req.empresaId, req.params.empresaId);
    const vinculo = await UsuarioEmpresaService.inativarVinculo(
      req.params.id,
